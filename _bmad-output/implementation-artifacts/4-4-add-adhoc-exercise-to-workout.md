...
- [ ] Task 1: Create Exercise Picker Modal (Architecture Decision)
  - [ ] Create `src/components/workout/ExercisePicker.tsx`
  - [ ] Implement search functionality and category grouping
- [ ] Task 2: Implement "Add Exercise" Action (AC: #1, #3)
  - [ ] Add the action button to `LogWorkoutScreen`
  - [ ] Implement the logic to add the selected exercise to the session state
- [ ] Task 3: Visual Differentiation (AC: #4)
  - [ ] Update `ExerciseCard.tsx` to handle an `isAdHoc` prop
  - [ ] Add a subtle "Ad-Hoc" or "Added" badge/indicator
- [ ] Task 4: Verification (AC: #2, #5, #6)
  - [ ] Verify search performance within the picker
  - [ ] Verify that ad-hoc exercises persist during the session

## Dev Notes

### UI/UX
- Use a `Modal` or a bottom sheet for the exercise picker.
- Re-use filtering logic from Story 2.5 if possible.
